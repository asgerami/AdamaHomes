...
,
    ...
    async getBookingsByUser(userId) {
        try {
            const bookings = await Booking.find({ user: userId }).populate('property').populate('user').populate('location').populate('payment');
            return bookings;
        } catch (error) {
    ...
    async getAllBookings() {
        try {
            const bookings = await Booking.find()
                ...
                .populate({
                    path: 'property',
                    select: 'name image price transactionType propertyStatus owner ',
                    populate: {
                        path: 'owner',
                        select: 'fullName', // Populate owner fullName
                    },
                    populate: {
                        path: 'location',
                        select: 'street city state country', // Populate owner fullName
                    },
                ...
                .populate({
                    path: 'location',
                    select: 'street city state country' // Adjust according to your schema
                })
                .populate({
                    path: 'payment',
                    select: 'status amount' // Adjust according to your schema
                });
            return bookings;
        } catch (error) {
