...
module.exports = {
    ...
    async getPropertyById(propertyId) {
        try {
            ...
            const property = await Property.findById(propertyId).populate('category').populate('location').populate('owner');
            if (!property) {
    ...
    async getAllProperties() {
        try {
            ...
            const properties = await Property.find().populate('category').populate('location').populate('owner');
            return properties;
        } catch (error) {
...
async  getPropertiesByCategory(categoryId) {
    try {
      // Find properties that match the category ID
      const properties = await Property.find({ category: categoryId }).exec();
      return properties;
    } catch (error) {
      // Rethrow error to be caught in the controller
